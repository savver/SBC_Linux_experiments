...
# *v.1.3.sh  use total cpu cores load, not individual for each core
#
# --- ~$ sensors ------------------------
...
max_num=-1
for f in "$LOG_DIR"/temperature_total_[0-9][0-9][0-9][0-9]_*.txt; do
    if [[ -f "$f" ]]; then
        base=$(basename "$f")
        num=$(echo "$base" | cut -d'_' -f3)   # format: temperature_total_0001_...
        if [[ $num =~ ^[0-9]{4}$ ]]; then
...
filename="$LOG_DIR/temperature_total_${num_str}_${timestamp}.txt"

echo "new log file is created: $filename"

# --- cacl cpu load in percentages, based on  /proc/stat, total on all cores ---
# out string "total_user=0.0;total_nice=0.0;..." 
#
calc_total_stats() {
    local line1="$1"   # 1st snapshot
    local line2="$2"   # 2d snapshot
...
#--- main cycle --------------------------------------------------------
while true; do
    # ---- 1st snapshot for cpu load stat ----
    stat1_cpu=$(cat /proc/stat | grep '^cpu ')

    sleep "$SLEEP_INTERVAL"

    # ---- 2d snapshot for cpu load stat ----
    stat2_cpu=$(cat /proc/stat | grep '^cpu ')
    ...
    # ---- total cpu load stat ----
    total_stats=$(calc_total_stats "$stat1_cpu" "$stat2_cpu")
    # Разбиваем на отдельные поля (с завершающей ;)
    IFS=';' read -ra stats_fields <<< "$total_stats"
